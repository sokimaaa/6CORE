...
public class PutOnHoldService implements PutOnHoldUseCase {
  ...
  public Mono<PutOnHoldResponse> reserve(PutOnHoldRequest putOnHoldRequest) {
    ...
    return inventoryPersistentPort
        ...
        .flatMap(
            inventory -> {
              ReservationRequest reservationRequest =
                  getReservationRequest(inventory, neededQuantity);
              reservationPersistentPort.save(reservationRequest);
              return inventoryPersistentPort.update(inventory);
            })
        .map(inventory -> getSuccessPunOnHoldResponse(inventory, neededQuantity))
        .defaultIfEmpty(getFailedPunOnHoldResponse(putOnHoldRequest));
  }

  public Mono<Void> releaseReservations() {
    return reservationPersistentPort
        .getReservationsForRelease()
        .flatMap(
            reservationResponse ->
                inventoryPersistentPort
                    .findByProductId(reservationResponse.getProductId())
                    .map(
                        inventoryFromDb ->
                            getInventoryWithPlusQuantity(
                                inventoryFromDb, reservationResponse.getQuantity())))
  ...
  private InventoryV01Impl getInventoryWithSubstrQuantity(
      InventoryV01Impl inventory, Integer neededQuantity) {
  ...
  private PutOnHoldResponse getSuccessPunOnHoldResponse(
      InventoryV01Impl inventory, Integer neededQuantity) {
  ...
  private InventoryV01Impl getInventoryWithPlusQuantity(
      InventoryV01Impl inventory, Integer quantityForRelease) {
    return ImmutableInventoryV01Impl.builder()
        .actualQuantity(inventory.getActualQuantity())
