...
public interface CheckoutShoppingCartReactiveRepository
    extends R2dbcRepository<ShoppingCart, String> {
  @Query("SELECT s FROM ShoppingCart s WHERE s.id = ?1")
  Mono<ShoppingCart> findByCartId(CartId id);

  @Query("UPDATE ShoppingCart s SET s.productIds = null WHERE s.id = ?1")
