...
@Tag(
    name = "Order Receive Specification",
    description = "REST reactive specification for interaction with Order service")
public interface OrderGetApi {
  ...
  @Operation(
      ...
      description = "Get all orders with all statuses by a single user")
  @ApiResponses(
      value = {
        @ApiResponse(
            responseCode = "200",
            ...
            content = {
              @Content(
                  ...
                  schema = @Schema(implementation = OrderShortResponse.class))
            }),
        @ApiResponse(
            responseCode = "401",
  ...
  default Flux<ResponseEntity<OrderDetailsResponse>> getOrder(
    ...
    OrderDetailsResponse mockResponse =
        new OrderDetailsResponse("-1", "-1", mockPrice, Set.of(mockItem), "-1");
    return Flux.just(ResponseEntity.ok(mockResponse));
  }
}
